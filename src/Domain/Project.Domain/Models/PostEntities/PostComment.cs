    ...
    public class PostComment : BaseEntity<Guid>, ISerializable
    {
        ...
        public PostComment(string email, string fullName, string content, bool isApproved)
        {
            if (string.IsNullOrEmpty(email))
                throw new AppException(Project.Domain.Shared.ApiResultStatusCode.BadRequest, "ایمیل الزامی است", System.Net.HttpStatusCode.BadRequest);

            if (string.IsNullOrEmpty(content))
                throw new AppException(ApiResultStatusCode.BadRequest, "متن نظر الزامی است", HttpStatusCode.BadRequest);
