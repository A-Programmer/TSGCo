...
namespace Project.Auth.Controllers.Account
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (await _usersService.AreUserCredentialsValidAsync(model.Username, model.Password))
                {
                    ...
                    claims.Add(new UserClaim(ClaimTypes.Name, model.Username));
                    foreach(var role in roles)
                    {
                        claims.Add(new UserClaim(ClaimTypes.Role, role));
                    }
                    ...
                    Console.WriteLine($"\n\n\n\n\nSignin LockedOut: {signInResult.IsLockedOut}\n\n\n\n");
                    Console.WriteLine($"\n\n\n\n\nSignin NotAllowed: {signInResult.IsNotAllowed}\n\n\n\n");
                    Console.WriteLine($"\n\n\n\n\nSignin TowFactor: {signInResult.RequiresTwoFactor}\n\n\n\n");
        ...
        public async Task<IActionResult> Logout(string logoutId)
        {
            ...
            // var user = new User("kamran");
            // // var role = new Role() { Name = "adm" };
            // // var roleResult = await _roleManager.CreateAsync(role);
            // var userPassword = await _usersService.GetUserByUsernameAsync("adm");
            // var upResult = await _userManager.AddPasswordAsync(userPassword, "adm");
            // var userResult = await _userManager.CreateAsync(user, "kamran");

            // var addToRoleResult = await _userManager.AddToRoleAsync(user, "adm");

            var isUser = await _userManager.FindByNameAsync(HttpContext.User.Identity.Name);
            if(isUser == null)
            {
                Console.WriteLine("\n\n\n\n\n Null \n\n\n\n\n");
            }
            ...
            var rr = HttpContext.User.IsInRole("adm");
