...
namespace Project.Auth.Services
{
    ...
    public class PersistedGrantStore : IPersistedGrantStore
    {
        ...
        public Task<IEnumerable<IdentityServer4.Models.PersistedGrant>> GetAllAsync(string subjectId)
        {
            ...
            var persistedGrants = _persistedGrants.Where(x => x.UserId == userId).ToList();
        ...
        {
            var convertedKey = Guid.Parse(filter.SubjectId);
            var persistedGrants = _persistedGrants.Where(x => x.UserId == convertedKey).ToList();
            var model = persistedGrants.Select(x => x.ToModel());

            _logger.LogDebug("{persistedGrantCount} persisted grants found for {subjectId}", persistedGrants.Count, filter.SubjectId);
        ...
        public Task RemoveAllAsync(PersistedGrantFilter filter)
        {
            ...
            {
                _logger.LogInformation("exception removing {persistedGrantCount} persisted grants from database for subject {subjectId}, clientId {clientId}, grantType {persistedGrantType}: {error}", persistedGrants.Count, filter.SubjectId, filter.ClientId, filter.Type, ex.Message);
            }
